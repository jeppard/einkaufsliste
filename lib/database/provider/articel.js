...
module.exports.addArticle = async function addArticle(article) {
    let conn;
    let res;
    try {
        conn = await pool.getConnection();
        res = await conn.query("INSERT INTO " + ARTICELS_TABLE_NAME + " (UserID, Name, Description, Type) VALUES (?, ?, ?, ?);", [article.userID, article.name, article.description, article.type]);
    } catch (err) {
        // TODO Add result
    ...
    try {
        ...
        if (articles != null && elements != null) {
            elements.forEach(e => {
                ...
                if (article != null) {
                    res.push(new ListElement(new Article(article.ID, article.Name, article.Type), e.Count, e.UnitType));
                }
            });
        }
    } catch (err) {
